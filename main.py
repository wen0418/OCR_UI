    ...
    def run(self):
        try:
            ...
            if result_list and len(result_list) > 0:
                ...
                print(f"📐 解析到 {len(boxes)} 個框, {len(texts)} 段文字")
...
class OcrMainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.is_pdf = False
        self.temp_img_path = ""
        self.original_file_path = ""  # 原始檔案路徑
        self.ocr_image_size = None     # OCR 辨識用的圖片實際尺寸 (width, height) 
        self.raw_ocr_results = []      # 儲存最原始的 OCR 辨識結果
    ...
    def run_ocr(self):
        ...
        # 統一將 UI 畫布上的 QPixmap 存成暫存檔供 OCR 辨識
        temp_dir = tempfile.gettempdir()
        self.temp_img_path = os.path.join(temp_dir, f"ocr_target_{uuid.uuid4().hex}.png")
        ui_pixmap = self.current_pixmap_item.pixmap()
        
        ui_pixmap.save(self.temp_img_path, "PNG")
        
        self.ocr_image_size = (ui_pixmap.width(), ui_pixmap.height())
        ocr_target = self.temp_img_path

        print(f"📐 OCR 目標圖片: {ocr_target}")
        print(f"📐 UI 與 OCR 圖片統一尺寸: {self.ocr_image_size[0]}x{self.ocr_image_size[1]}")

        self.ocr_thread = OCRWorker(ocr_target)
    ...
    def update_bboxes(self):
        if not self.raw_ocr_results or not self.current_pixmap_item:
            return
        ...
        # --- 1. 取得原圖尺寸與原圖中心點 ---
        ui_pixmap = self.current_pixmap_item.pixmap()
        display_w = ui_pixmap.width()
        display_h = ui_pixmap.height()
        
        image_center_x = display_w / 2.0
        image_center_y = display_h / 2.0

        # 🛠️ [DEBUG 標記 1] 畫出「原圖的絕對中心點」 (綠色大圓點)
        img_center_dot = QtWidgets.QGraphicsEllipseItem(image_center_x - 6, image_center_y - 6, 12, 12)
        img_center_dot.setBrush(QtGui.QBrush(QtGui.QColor(0, 255, 0, 180))) # 半透明綠色
        img_center_dot.setPen(QtGui.QPen(QtCore.Qt.NoPen))
        img_center_dot.setZValue(10)
        img_center_dot.setParentItem(self.current_pixmap_item)
        self.bbox_items.append(img_center_dot)

        # --- 2. 找出「整個文字裁剪區域」的上下左右極限 ---
        min_x = float('inf')
        max_x = float('-inf')
        min_y = float('inf')
        max_y = float('-inf')

        for res in self.raw_ocr_results:
            for pt in res["poly"]:
                x = float(pt[0])
                y = float(pt[1])
                if x < min_x: min_x = x
        ...
        # 計算出文字群聚的原始中心點
        crop_region_center_x = (min_x + max_x) / 2.0
        crop_region_center_y = (min_y + max_y) / 2.0

        # --- 3. 左上角原點的縮放與偏移 ---
        # 💡 利用這四個數字來進行微調，解決「越往下/右，偏移越大」的問題
        off_x = 48
        off_y = 50
        scale_x = 0.8
        scale_y = 0.8

        # 🛠️ [DEBUG 標記 2] 畫出「文字群聚中心點」 (藍色大圓點)
        # 注意：藍點也必須跟著套用縮放與偏移，才會出現在正確的對應位置上
        mapped_crop_cx = (crop_region_center_x * scale_x) + off_x
        mapped_crop_cy = (crop_region_center_y * scale_y) + off_y

        crop_center_dot = QtWidgets.QGraphicsEllipseItem(mapped_crop_cx - 6, mapped_crop_cy - 6, 12, 12)
        crop_center_dot.setBrush(QtGui.QBrush(QtGui.QColor(0, 0, 255, 180))) # 半透明藍色
        crop_center_dot.setPen(QtGui.QPen(QtCore.Qt.NoPen))
        crop_center_dot.setZValue(10)
        crop_center_dot.setParentItem(self.current_pixmap_item)
        self.bbox_items.append(crop_center_dot)

        # --- 4. 畫出所有文字的辨識框 ---
        for i, res in enumerate(self.raw_ocr_results):
            poly_points = res["poly"]
            qpolygon = QtGui.QPolygonF()
            
            for pt in poly_points:
                original_x = float(pt[0])
                original_y = float(pt[1])
                
                # 核心數學：回歸以左上角 (0,0) 為原點的單純乘法與加法
                mapped_x = (original_x * scale_x) + off_x
                mapped_y = (original_y * scale_y) + off_y
